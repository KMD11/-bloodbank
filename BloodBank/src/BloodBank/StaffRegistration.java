...
import BloodBank.CheckingOfJava;
...
public class StaffRegistration {
	...
	private void initialize() {
		...
		lblStiffID.setBounds(61, 33, 149, 21);
		...
		lblIdentificationNumber.setBounds(61, 199, 149, 21);
		panel_1.add(lblIdentificationNumber);
		
		JLabel lblAddress = new JLabel("Address:");
		lblAddress.setFont(new Font("Tahoma", Font.BOLD, 13));
		lblAddress.setBounds(61, 243, 149, 21);
		panel_1.add(lblAddress);
		
		JLabel lblEmail = new JLabel("Email:");
		lblEmail.setFont(new Font("Tahoma", Font.BOLD, 13));
		lblEmail.setBounds(61, 281, 149, 21);
		...
		txtRoll.setFont(new Font("Tahoma", Font.BOLD, 13));
		...
		lblRoll.setBounds(61, 373, 149, 21);
		panel_1.add(lblRoll);
		...
		txtCon_Pass.setFont(new Font("Tahoma", Font.BOLD, 13));
		txtCon_Pass.setBounds(234, 160, 128, 20);
		panel_1.add(txtCon_Pass);
		
		JLabel lblPhoneNumber = new JLabel("Phone Number:");
		lblPhoneNumber.setFont(new Font("Tahoma", Font.BOLD, 13));
		lblPhoneNumber.setBounds(61, 326, 149, 21);
		panel_1.add(lblPhoneNumber);
		
		txtPhNo = new JTextField();
		txtPhNo.setFont(new Font("Tahoma", Font.BOLD, 13));
		...
		btnRegister.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(CheckingOfJava.IsNull(txtName.getText()))
				{
					JOptionPane.showMessageDialog(null, "First you must enter Staff Name.");
					txtName.requestFocus();
					txtName.selectAll();
				}
				else if(CheckingOfJava.IsNull(txtPassword.getText()))
				{
					JOptionPane.showMessageDialog(null, "You must enter Password.");
					txtPassword.requestFocus();
					txtPassword.selectAll();
				}
				else if(CheckingOfJava.IsNull(txtCon_Pass.getText()))
				{
					JOptionPane.showMessageDialog(null, "You must enter Confirm Password.");
					txtCon_Pass.requestFocus();
					txtCon_Pass.selectAll();
				}
				else if(CheckingOfJava.IsNull(txtNrc.getText()))
				{
					JOptionPane.showMessageDialog(null, "You must enter Identification Number");
					txtNrc.requestFocus();
					txtNrc.selectAll();
				}
				else if(CheckingOfJava.IsNull(txtAddress.getText()))
				{
					JOptionPane.showMessageDialog(null, "You must enter Address");
					txtAddress.requestFocus();
					txtAddress.selectAll();
				}
				else if(CheckingOfJava.IsNull(txtEmail.getText()))
				{
					JOptionPane.showMessageDialog(null, "You must enter Email Address");
					txtEmail.requestFocus();
					txtEmail.selectAll();
				}
				else if(CheckingOfJava.IsNull(txtPhNo.getText()))
				{
					JOptionPane.showMessageDialog(null, "You must enter Phone Number");
					txtPhNo.requestFocus();
					txtPhNo.selectAll();
				}
				else if(CheckingOfJava.IsNull(txtRoll.getText()))
				{
					JOptionPane.showMessageDialog(null, "You must enter Roll type");
					txtRoll.requestFocus();
					txtRoll.selectAll();
				}
				else if(txtPassword.getPassword().length < 6)
				{
					JOptionPane.showMessageDialog(null, "Your password is least 6 characters");
					txtPassword.requestFocus();
					txtPassword.selectAll();
				}
				...
				else
				{
					...
					else
					{
						...
						boolean register=msql.insertData("staff", str);
						if(register)
						{
